...
The pin is **PUBLIC upstream `openemr/openemr` `rel-810`**. This module uses only public-core APIs, and rel-810's `CsrfUtils` is byte-identical to the internal `oce-810` core, so analysis resolves the 8.1 Symfony-Session signatures identically. Because the source is public, no GitHub App token / credential is needed — `composer install` here works unauthenticated, in CI and locally.
